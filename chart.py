...
def getMetrics(c,type):
	
	metrics = Metric.objects.all().filter(company=c).filter(category=type).order_by('date')
					...
					for m in metrics:
						...
						if m.date.year == y and m.date.month == i:
							...
							if not oldStat: 
								...
								#oldStat = m
								#ticker = False
							elif m.date.month == oldStat.date.month and int(m.stat) <= int(oldStat.stat): 
								break
							else:
								stats.append(int(m.stat))
								#oldStat = m
